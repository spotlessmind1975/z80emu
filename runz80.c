...
int main(int argc, char **argv)
{
        ...
        while (++argv, --argc > 0)
        {
                ...
                if (!strcmp(*argv, "-h"))
                        n = doHelp(argc, argv, &context);
                else if (!strcmp(*argv, "-l"))
                        n = doLoad(argc, argv, &context);
                else if (!strcmp(*argv, "-R"))
                        n = doRST(argc, argv, &context);
                else if (!strcmp(*argv, "-L"))
                        n = doLabels(argc, argv, &context);
                else if (!strcmp(*argv, "-Li"))
                        n = doInspections(argc, argv, &context);
                        ...
                        n = doMsx1(argc, argv, &context);
                else if (!strcmp(*argv, "-u"))
                        n = doListing(argc, argv, &context);
                else if (!strcmp(*argv, "-p"))
                        n = doProfiling(argc, argv, &context);
        ...
        if (context.output_filename)
        {
                ...
                {
                        if (
                            strcmp(context.label_name[i], "WORKING") == 0 || strcmp(context.label_name[i], "TEMPORARY") == 0 || strcmp(context.label_name[i], "DESCRIPTORS") == 0)
                        {
                                fprintf(handle, "%s\n", context.label_name[i]);
                                if (strcmp(context.label_name[i], "WORKING") == 0 || strcmp(context.label_name[i], "TEMPORARY") == 0)
                                {
                                        max = 1024;
                ...
                {
                        if (
                            strcmp(context.label_name[i], "WORKING") == 0 || strcmp(context.label_name[i], "TEMPORARY") == 0 || strcmp(context.label_name[i], "DESCRIPTORS") == 0 || strcmp(context.label_name[i], "USING") == 0)
                        {
                        }
                        else
                        {
